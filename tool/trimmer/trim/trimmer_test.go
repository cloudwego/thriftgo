...
func TestTrimASTWithCompose(t *testing.T) {
	testcases := []struct {
		...
		expect     func(t *testing.T, ast *parser.Thrift, structureTrimmed int, fieldTrimmed int, err error)
	}{
		{
			...
			expect: func(t *testing.T, ast *parser.Thrift, structureTrimmed int, fieldTrimmed int, err error) {
				...
				test.Assert(t, structureTrimmed == 0)
				test.Assert(t, fieldTrimmed == 0)
	...
	for _, tc := range testcases {
		t.Run(tc.desc, func(t *testing.T) {
			arg := tc.composeArg()
			st, ft, err := TrimASTWithCompose(arg)
			tc.expect(t, arg.TargetAST, st, ft, err)
		})
	}
}
