...
{{- $TypeName := .GoName}}
func (p *{{$TypeName}}) Read({{ctx "iprot thrift.TProtocol"}}) (err error) {
	...
	for {
		{{if Features.KeepUnknownFields}}name{{else}}_{{end}}, fieldTypeId, fieldId, err = iprot.ReadFieldBegin({{ctx}})
		if err != nil {
		...
		case {{.ID}}:
			if fieldTypeId == thrift.{{.Type | GetTypeIDConstant }} {
				if err = p.{{.Reader}}({{ctx "iprot"}}); err != nil {
					goto ReadFieldError
				}
				{{- if .Requiredness.IsRequired}}
				isset{{.GoName}} = true
...
{{- range .Fields}}
{{$FieldName := .GoName}}
func (p *{{$TypeName}}) {{.Reader}}({{ctx "iprot thrift.TProtocol"}}) error {
	{{- $ctx := MkRWCtx .}}
...
{{- $TypeName := .GoName}}
func (p *{{$TypeName}}) Write({{ctx "oprot thrift.TProtocol"}}) (err error) {
	{{- if gt (len .Fields) 0 }}
	var fieldId int16
	{{- end}}
	{{- if eq .Category "union"}}
	...
	{{- end}}
	if err = oprot.WriteStructBegin({{ctx (Quote .Name)}}); err != nil {
		goto WriteStructBeginError
	}
	if p != nil {
		{{- range .Fields}}
		if err = p.{{.Writer}}({{ctx "oprot"}}); err != nil {
			fieldId = {{.ID}}
...
func (p *{{$TypeName}}) {{.Writer}}({{ctx "oprot thrift.TProtocol"}}) (err error) {
	{{- if .Requiredness.IsOptional}}
	if p.{{$IsSetName}}() {
	{{- end}}
	if err = oprot.WriteFieldBegin({{ctx (Quote .Name)}}, thrift.{{$TypeID}}, {{.ID}}); err != nil {
...
{{define "FieldReadStructLike"}}
	...
	if err := {{.Target}}.Read({{ctx "iprot"}}); err != nil {
		return err
	}
...
{{define "FieldReadBaseType"}}
	...
	var {{.Target}} {{.TypeName}}
	{{- end}}
	if v, err := iprot.Read{{.TypeID}}({{ctx}}); err != nil {
		return err
	} else {
	{{- if .IsPointer}}
		{{- if $DiffType}}
...
{{define "FieldWriteStructLike"}}
	if err := {{.Target}}.Write({{ctx "oprot"}}); err != nil {
		return err
	}
...
{{- if .Type.Category.IsBinary}}{{$Value = printf "[]byte(%s)" $Value}}{{end}}
	if err := oprot.Write{{.TypeID}}({{ctx $Value}}); err != nil {
		return err
	}
...
{{define "FieldWriteMap"}}
	if err := oprot.WriteMapBegin({{ctx ""}}thrift.
...
{{define "FieldWriteSet"}}
		if err := oprot.WriteSetBegin({{ctx ""}}thrift.
...
{{define "FieldWriteList"}}
		if err := oprot.WriteListBegin({{ctx ""}}thrift.
		{{- .ValCtx.Type | GetTypeIDConstant -}}
		, len({{.Target}})); err != nil {
			return err
		}
